...
function pathsOf(text, baseUrl, source) {
...
function webpackAliasesOf(text, source) {
    const rules = [];
    ...
    if (aliasKey >= 0) {
        ...
        if (block !== undefined) {
            ...
            while (match !== null) {
                ...
                if (name !== '' && target !== '')
                    rules.push({ prefix: name, exact: true, targets: [target], source });
                match = entry.exec(block);
            }
        }
    }
 ...
 */
export async function aliasTable(workspace) {
    ...
    for (const name of WEBPACK_CANDIDATES) {
        ...
        for (const root of webpackRootsOf(text))
            roots.add(root);
    }
    // `src` is the overwhelmingly common source root; try it when it exists.
    if (await isDirectory(join(workspace, 'src')))
        roots.add('src');
    const table = { rules, roots: [...roots] };
 ...
 */
export function aliasCandidates(specifier, table) {
    const out = [];
    ...
    for (const rule of sorted) {
        if (rule.exact) {
            ...
            for (const target of rule.targets)
                out.push({ base: target, source: rule.source });
            continue;
        }
    ...
    for (const root of table.roots)
        out.push({ base: `${root}/${specifier}`.replace(/^\.\//, ''), source: root === 'src' ? 'src 根目录' : `模块根 ${root}` });
    return out;
}
/** Directory names read at the workspace root, for a quick "does this top-level dir exist" probe. */
export async function topLevelDirectories(workspace) {
