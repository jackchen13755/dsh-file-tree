...
- 实测：**DSH 0.1.6-alpha.2**（macOS，web profile）——真机验证覆盖：PNG / SVG 点开即渲染（SVG 按 120×60 真实尺寸）、文本类走官方标签着色、切换标签后展开状态保留、指南入口出现、树展开、官方图标（10/10 行）、缩进导轨、悬停操作、搜索结果显示所在目录、`＠` 插入后输入框出现 `@<相对路径>` 且被渲染为**引用芯片**、点文件行在标签页用**官方预览**打开（12 种 token 颜色 + 行号）、行内 `▤` 预览同样着色、控制台 0 报错。
- 声明下限：`dsh >= 0.1.5-rc.1`。
