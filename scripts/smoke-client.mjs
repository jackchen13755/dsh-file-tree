...
check('bundle id is the package name', loaded?.id === 'dsh-file-tree')
...
check('tab kind is files', tabDefinitions[0]?.kind === 'files')
...
check('inject factory carries the tab opener', typeof injected?.openResource === 'function')
// Without a sidebar controller the opener must report failure (the panel then
// falls back to its inline preview) instead of throwing.
const opened = injected?.openResource?.('a/b.txt')
check('opener reports a missing controller', opened?.ok === false && typeof opened?.reason === 'string')
check('opener builds the product address', String(opened?.address).startsWith('dsh-resource://file/session/session-1/'))
