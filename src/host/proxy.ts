 ...
 */
function splitPrefix(pathname: string): { id: string; rest: string } | undefined {
  if (pathname !== CODE_SERVER_PREFIX && !pathname.startsWith(`${CODE_SERVER_PREFIX}/`)) return undefined
...
function forwardableHeaders(request: IncomingMessage): Record<string, string | string[]> {
 ...
 */
export function proxyHttp(request: IncomingMessage, response: ServerResponse, pathname: string): boolean {
 ...
 */
export function proxyUpgrade(request: IncomingMessage, socket: Duplex, head: Buffer): boolean {
  let pathname: string
  try {
 ...
 */
export function interceptUpgrades(ctx: Context): () => void {
  const server = rawServer(ctx)
  if (server === undefined) {
    ctx.logger?.warn?.(
      new Error('dsh-file-tree: webServer exposes no raw http server; the code-server panel will load but its sockets will not connect'),
    )
    return () => {}
  }
  const original = server.emit
  const patched = function patchedEmit(this: Server, event: string, ...args: unknown[]): boolean {
    if (event === 'upgrade' && args.length >= 2) {
      const request = args[0] as IncomingMessage
      const socket = args[1] as Duplex
      const head = (args[2] ?? Buffer.alloc(0)) as Buffer
      try {
        if (proxyUpgrade(request, socket, head)) return true
      } catch (error) {
        ctx.logger?.warn?.(error instanceof Error ? error : new Error(String(error)))
      }
    }
    return original.apply(this, [event, ...args] as Parameters<Server['emit']>)
  } as Server['emit']
  server.emit = patched
  return () => {
    if (server.emit === patched) server.emit = original
  }
}

/** The underlying `http.Server` behind the host web server, when it exposes one. */
function rawServer(ctx: Context): Server | undefined {
  const candidate = (webServerOf(ctx) as unknown as { server?: unknown }).server
  if (candidate === undefined || candidate === null) return undefined
  if (typeof (candidate as { emit?: unknown }).emit !== 'function') return undefined
  return candidate as Server
}
