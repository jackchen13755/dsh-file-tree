...
import { aliasCandidates, topLevelDirectories, type AliasTable } from './aliases.js'
...
export async function resolveSpecifier(
  ...
  specifier: string,
  table: AliasTable,
): Promise<{ path: string | null; rule?: string; reason?: string }> {
  const raw = specifier.trim()
  if (raw === '') return { path: null, reason: 'empty' }

  const bases: Array<{ base: string; rule: string }> = []
  ...
  for (const { base, rule } of bases) {
    ...
    if (resolved !== null) return { path: resolved, rule }
  }
...
function normalizeFrom(directory: string, specifier: string): string {
...
async function resolveBase(workspace: string, base: string): Promise<string | null> {
  const slash = base.lastIndexOf('/')
  const dot = base.lastIndexOf('.')
  const stem = dot > slash ? base.slice(0, dot) : base
  const candidates = new Set<string>([base])
  for (const extension of RESOLVE_EXTENSIONS) {
    if (extension !== '') candidates.add(`${base}${extension}`)
  }
  ...
  for (const index of RESOLVE_INDEXES) {
    ...
    if ((await fileStatus(workspace, candidate)) === 'file') return candidate
  }
  return null
}

/** `file`, `directory` or `missing`, resolved inside the workspace. */
async function fileStatus(workspace: string, candidate: string): Promise<'file' | 'directory' | 'missing'> {
